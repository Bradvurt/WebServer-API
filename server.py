...
class User():
	...
	def add_item(item_id):
		...
		if item_id in products:
			return {
				"ok": True,
				"message": "Этот элемент уже добавлен!"
			}
...
class GetPage():
	...
	def register():
		...
		if reg["ok"]:
			...
			if session["ok"]:
				resp = GetPage.success("Поздравляем, теперь вы официально алхимик!", isAuth=1)
	...
	def auth():
		...
		session = User.start_session(
			login, password, str(request.remote_addr))
		if session["ok"]:
			resp = GetPage.success("Отлично! Личность установлена, проходи, мой друг!", isAuth=1)
			resp.set_cookie("Auth", session["message"])
			return resp
		return GetPage.error(session["message"])
...
def cart():
	if User.isAuth():
		...
		return GetPage.error("Сначала пройди авторизацию!")
...
def edit_item(id):
	...
	if not isOwner:
		return GetPage.error("Нет доступа! Если не пройдете идентификацию, придется вызвать голема!")
